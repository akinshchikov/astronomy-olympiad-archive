...
Отслеживаемые manifest-файлы и индексы — намеренные публичные артефакты: это
последний проверенный глобальный snapshot репозитория, тогда как промежуточное
локальное состояние pipeline остаётся игнорируемым.

Тяжёлые локальные бинарные данные намеренно не коммитятся вне этого узкого
исключения:
...
data/
  ...
  preserved/             # исключительные исторические публикации с checksum-manifest
```
...
Запуск `run_pipeline.py --families ...` защищает публичный snapshot: он может
обновлять локальные raw/archive/checkpoint-данные выбранных семейств, но при
завершении восстанавливает отслеживаемые глобальные manifest-файлы и индексы
байт-в-байт.
...
- Точечная очистка удаляет строки выбранных семейств только из локальных
  download/checkpoint/normalization-manifest. Отслеживаемый публичный discovery
  snapshot она намеренно не меняет; `data/archive/objects/` также не удаляется.
...
- Запуск с `--families ...` предназначен для локального acquisition/validation и
  никогда не публикует частичный snapshot. Отслеживаемые глобальные manifests и
  индексы обновляет только прогон без `--families`.
...
- `russia_correspondence_preserved_publication` — сознательное исключение из обычной publishing policy: четыре русских PDF «задачи с решениями» Российской Открытой Заочной Школьной Астрономической Олимпиады за 2005–2008 годы долго были публичны, затем исчезли по техническим причинам и явно разрешены к повторной публикации. В Git попадают только сами четыре файла и checksum-manifest; частная переписка о provenance не публикуется.
...
- Книги, доступные только для покупки, не являются целями скачивания. Публичный
  документ, на который прямо ссылается ограниченная официальная страница материалов
  олимпиады, можно скачать и с внешнего хоста; рекурсивно обходить этот внешний сайт
  crawler не будет.
...
Текущий публичный snapshot по коммитимым артефактам обновлён на `2026-09-25`:

- настроенные seed-источники: `55`
- обнаруженные публичные документы: `4057`
- строки в `olympiads_index.csv`: `797`
- уникальные публичные файлы в `files_index.csv`: `4179`
- relation groups: `740`
...
Этот репозиторий подготовлен так, чтобы на GitHub выкладывать код и лёгкие метаданные, плюс узкое исключение `data/preserved/`, описанное в [PUBLISHING.md](PUBLISHING.md); полный общий бинарный архив по-прежнему пересобирается локально.
